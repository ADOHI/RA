...
public class CameraWalkerController : BasicWalkerController {
	...
	protected override Vector3 CalculateMovementDirection()
	{
        ...
        if (gyroInput)
        {
            _horizontalInput = InputQueueManager.Instance.inputX;
            _verticalInput = 1f;
        }
        else
        {
		...
		_direction += cameraControls.transform.forward * _verticalInput;
		_direction += cameraControls.transform.right * _horizontalInput;

		//Clamp movement vector to magnitude of '1f';
		if(_direction.magnitude > 1f)
			_direction.Normalize();

        InputQueueManager.Instance.MoveMirrorImage(_direction);
        return _direction;
	}
